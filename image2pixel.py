...
def change_block(x, y, black_size, img_array):
  ...
  max_color = list(map(int, max_color[1:len(max_color) - 1].split(",")))

  new_color = []
  for each_color in max_color:
    if each_color > 128:
      res = 255
    else:
      res = 0
    new_color.append(res)
  # max_color = tuple(new_color)
  
  # print(type(max_color))
  # 将区块内所有的颜色值设置为颜色最多的颜色
  for a in block_pos_list:
    img_array[a[0], a[1]] = tuple(new_color)
